...
const data = [
  {
    ...
    slug: '/',
    icon: (
      <LocationIcon className="w-12 lg:w-13 xl:w-15 h-12 lg:h-13 xl:h-15" />
    ),
  ...
  },
  {
    ...
    slug: '/',
    icon: (
      <PhoneIcon className="w-12 lg:w-13 xl:w-15 h-12 lg:h-13 xl:h-15" />
    ),
    name: 'text-phone',
  ...
  },
  {
    ...
    slug: '/',
    icon: (
      <MailIcon className="w-12 lg:w-13 xl:w-15 h-12 lg:h-13 xl:h-15" />
    ),
    name: 'text-email',
...
const ContactInformation = ({ lang }) => {
  const { t } = useTranslation(lang, 'common');
  
  return (
    <>
      {data.length > 0 && (
        <div className="px-5 lg:px-0 xl:px-12 sm:grid grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-7 max-w-355 mx-auto space-y-7 sm:space-y-0 pt-1">
          {data?.map((item) => (
            <div
